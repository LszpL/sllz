    ...
    <div class="register-box" style="position:absolute;top:-726px;right:290px;">
      ...
      {{csrf_field()}}
        ...
        <div class="cell vcode">
          ...
          <button type="submit" id="js-mobile_btn"  class="button btn-white1" style="padding:0 128px;">
          确认修改</button>
		  <a href="{{url('home/login')}}" style="position:absolute;bottom:27px;right: 33px;">返回登录></a>
        </form>
      </div>
  </div>
</div>
...
function phone(){
  ...
  if(res){
  $.post("{{url('/fphone')}}",{'_token':"{{csrf_token()}}",'phone':phone,'pwd':pwd,'repwd':repwd},function(data){
    layer.msg(data);
    if(data=='验证码发送成功'){
